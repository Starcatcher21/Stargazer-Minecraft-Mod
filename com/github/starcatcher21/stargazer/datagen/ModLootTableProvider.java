...
public class ModLootTableProvider extends FabricBlockLootTableProvider {
    ...
    public LootTable.Builder oreDrops(Block drop, Item raw, float min, float max) {
        RegistryEntryLookup impl = this.a.d(RegistryKeys.ENCHANTMENT);
        return this.dropsWithSilkTouch(drop, (LootPoolEntry.Builder)this.a(drop, ((LeafEntry.Builder)ItemEntry.builder(raw).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(min, max)))).apply(ApplyBonusLootFunction.oreDrops(impl.getOrThrow(Enchantments.FORTUNE)))));
    }
    public LootTable.Builder customLeavesDrop(Block drop, Item raw, float min, float max) {
        RegistryEntryLookup impl = this.a.d(RegistryKeys.ENCHANTMENT);
        return this.dropsWithSilkTouchOrShears(drop, (LootPoolEntry.Builder)this.a(drop, ((LeafEntry.Builder)ItemEntry.builder(raw).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(min, max)))).apply(ApplyBonusLootFunction.oreDrops(impl.getOrThrow(Enchantments.FORTUNE)))));
    }

    public LootTable.Builder conditionDrop(Block crop, Item product, LootCondition.Builder condition) {
        return this.a(crop, LootTable.builder().pool(LootPool.builder().with(((LeafEntry.Builder)ItemEntry.builder(product).conditionally(condition)))));
    }
