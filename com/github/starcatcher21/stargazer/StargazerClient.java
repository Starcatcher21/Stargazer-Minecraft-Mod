...
public class StargazerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            try {
                if (client != null && client.world != null && client.s != null) {
                    ...
                    if (client.s.getControllingVehicle() != null) {
                        if (!client.s.getControllingVehicle().isLiving()) {
                            if (Dimensions.REG_COSMIC_WORLD.getValue().equals(client.world.getRegistryKey().getValue())) {
                                client.s.getAttributeInstance(StargazerAttributes.DASH_LEVEL).addTemporaryModifier(PlayerCosmicGrav.dash_modifier);
                            }
                        }
                        if (client.s.getControllingVehicle().isLiving()) {
                            if (Dimensions.REG_COSMIC_WORLD.getValue().equals(client.world.getRegistryKey().getValue())) {
                                if (!client.s.getStatusEffects().contains(StatusEffects.COSMO)) {
                                    client.s.getAttributeInstance(StargazerAttributes.DASH_LEVEL).removeModifier(PlayerCosmicGrav.dash_modifier);
                                }
                            }
                        }
                    }
