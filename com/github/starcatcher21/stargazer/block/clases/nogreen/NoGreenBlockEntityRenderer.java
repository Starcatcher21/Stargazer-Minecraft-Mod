...
public class NoGreenBlockEntityRenderer<T extends NoGreenBlockEntity> implements BlockEntityRenderer<T, NoGreenBlockEntityRenderState> {
    public NoGreenBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    }
...
//        this.renderSide(entity, matrix, vertexConsumer, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, Direction.UP);
    ...
    protected RenderLayer getLayer() {
        return CustomRenderLayers.NO_GREEN;
    }

    @Override
    public NoGreenBlockEntityRenderState createRenderState() {
        return new NoGreenBlockEntityRenderState();
    }

    @Override
    public void render(NoGreenBlockEntityRenderState state, MatrixStack matrices, OrderedRenderCommandQueue queue, CameraRenderState cameraState) {
        queue.submitCustom(
                matrices,
                this.getLayer(),
    ...
    public void updateRenderState(
        ...
        for (Direction direction : Direction.values()) {
            try {
                if (!world.getBlockState(entity.getPos().offset(direction, 1)).getBlock().equals(ModBlock.NOGREEN_BLOCK)) {
