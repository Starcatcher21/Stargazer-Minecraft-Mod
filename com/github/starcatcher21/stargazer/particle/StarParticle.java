...
public class StarParticle extends BillboardParticle {
    ...
    private float angularVelocity = (float)Math.toRadians(this.y.nextBoolean() ? -30.0 : 30.0);
    private final float angularAcceleration = (float)Math.toRadians(this.y.nextBoolean() ? -5.0 : 5.0);
    ...
    protected StarParticle(
        ...
        float g = size * (this.y.nextBoolean() ? 0.05F : 0.075F);
        this.scale = g;
        this.setBoundingBoxSpacing(g, g);
        this.velocityMultiplier = 1.0F;
        this.velocityY = -initialYVelocity;
        float h = this.y.nextFloat();
    ...
    public void a() {
        ...
        if (!this.dead) {
            ...
            if (this.t || this.maxAge < 299 && (this.velocityX == 0.0 || this.velocityZ == 0.0)) {
                this.markDead();
            }
