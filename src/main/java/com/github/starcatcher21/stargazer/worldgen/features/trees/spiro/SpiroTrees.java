...
public class SpiroTrees extends Feature<TreeConfig> {
    ...
    public static Tree spiro = register("spiro");

    public SpiroTrees(Codec<TreeConfig> codec) {
        super(codec);
    }

    public static Tree register(String name) {
        Tree tree = new Tree(true, name, RedOrbBlocks.SPIRO_LOG.defaultBlockState().setValue(BlockStateProperties.AXIS, Direction.Axis.X), RedOrbBlocks.SPIRO_LEAVES.defaultBlockState());
        TREELIST.add(tree);
        return tree;
    }
    public static void init() {
        SpiroTree1.init(spiro);
    }
