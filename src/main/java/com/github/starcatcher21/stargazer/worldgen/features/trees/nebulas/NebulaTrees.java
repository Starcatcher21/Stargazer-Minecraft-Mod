...
public class NebulaTrees extends Feature<TreeConfig> {
    ...
    public static Tree BLUETREE = register("blue");
    public static Tree PURPLETREE = register("purple");
    public static Tree REDTREE = register("red");
    public static Tree YELLOWTREE = register("yellow");

    public NebulaTrees(Codec<TreeConfig> codec) {
        super(codec);
    }

    public static Tree register(String name) {
        Tree tree = new Tree(true, name, Nebulas.BLUE_NEBULA_LOG.defaultBlockState().setValue(BlockStateProperties.AXIS, Direction.Axis.Y), Nebulas.BLUE_NEBULA_LEAVES.defaultBlockState());
        TREELIST.add(tree);
        return tree;
    }
    public static void init() {
        Purple.init(PURPLETREE);
        Blue.init(BLUETREE);
        Red.init(REDTREE);
        Yellow.init(YELLOWTREE);
    }
