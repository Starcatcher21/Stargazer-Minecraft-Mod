...
public class FullMoonTrees extends Feature<TreeConfig> {
    ...
    public static Tree Moon = register("Moon");

    public FullMoonTrees(Codec<TreeConfig> codec) {
        super(codec);
    }

    public static Tree register(String name) {
        Tree tree = new Tree(true, name, MoonBlocks.FULL_MOON_LOG.defaultBlockState().setValue(BlockStateProperties.AXIS, Direction.Axis.Y), MoonBlocks.FULL_MOON_LEAVES.defaultBlockState());
        tree.addFruit(MoonBlocks.FULL_MOON_CORE.defaultBlockState());
        tree.setFruitChange(100);
        TREELIST.add(tree);
        return tree;
    }
    public static void init() {
        FullMoonTree1.init(Moon);
    }
