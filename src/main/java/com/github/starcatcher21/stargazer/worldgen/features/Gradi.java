...
public class Gradi extends Feature<DefaultFeatureConfig> {
    public Gradi(Codec<DefaultFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        for (int o = 0; o < origin.getY()/10; o++) {
            ...
            if (world.isValidForSetBlock(newori) && world.isAir(newori) && (neworiunderstate.equals(Darkness.DYLIUM.getDefaultState()) || neworiunderstate.equals(MoonBlocks.MOON_ROCK.getDefaultState()))) {
    ...
    private BlockState getState() {
        ...
        BlockState forgor = Darkness.GRADI.getDefaultState().with(FlowerbedBlock.HORIZONTAL_FACING, direction).with(FlowerbedBlock.FLOWER_AMOUNT, rand.nextInt(1,4));
        return forgor;
    }
    ...
    private ArrayList<BlockPos> next(StructureWorldAccess world, BlockPos pos) {
        for (int i = 0; i < 3; i++) {
            if (world.isValidForSetBlock(pos.down(i)) && world.isAir(pos.down(i))) {
                ...
                if (state.equals(MoonBlocks.MOON_ROCK.getDefaultState()) || state.equals(Darkness.DYLIUM.getDefaultState())) {
