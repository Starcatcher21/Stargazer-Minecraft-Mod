...
public class NoGreenBlockEntityRenderer<T extends NoGreenBlockEntity> implements BlockEntityRenderer<T, NoGreenBlockEntityRenderState> {
    public NoGreenBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {
    }
...
//        this.renderSide(entity, matrix, vertexConsumer, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, Direction.UP);
    ...
    @Override
    public void submit(NoGreenBlockEntityRenderState state, PoseStack matrices, SubmitNodeCollector queue, CameraRenderState cameraState) {
    ...
    public void extractRenderState(
        ...
        for (Direction direction : Direction.values()) {
            try {
                if (!world.getBlockState(entity.getBlockPos().relative(direction, 1)).getBlock().equals(ModBlock.NOGREEN_BLOCK)) {
