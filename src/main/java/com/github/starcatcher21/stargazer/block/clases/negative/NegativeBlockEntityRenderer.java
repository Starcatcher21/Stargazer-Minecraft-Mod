...
import com.github.starcatcher21.stargazer.block.clases.star.cosmic.CosmicBlockEntityRenderState;
...
//        this.renderSide(entity, matrix, vertexConsumer, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, Direction.UP);
    ...
    @Override
    public void render(NegativeBlockEntityRenderState state, MatrixStack matrices, OrderedRenderCommandQueue queue, CameraRenderState cameraState) {
        queue.submitCustom(
                matrices,
                this.getLayer(),
    ...
    public void updateRenderState(
        ...
        for (Direction direction : Direction.values()) {
            try {
                if (!world.getBlockState(entity.getPos().offset(direction, 1)).getBlock().equals(ModBlock.NEGATIVE_BLOCK)) {
