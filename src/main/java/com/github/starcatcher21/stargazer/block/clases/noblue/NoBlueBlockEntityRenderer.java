...
public class NoBlueBlockEntityRenderer<T extends NoBlueBlockEntity> implements BlockEntityRenderer<T, NoBlueBlockEntityRenderState> {
    public NoBlueBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    }
...
//        this.renderSide(entity, matrix, vertexConsumer, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, Direction.UP);
    ...
    protected RenderLayer getLayer() {
        return CustomRenderLayers.NO_BLUE;
    }

    @Override
    public NoBlueBlockEntityRenderState createRenderState() {
        return new NoBlueBlockEntityRenderState();
    }

    @Override
    public void render(NoBlueBlockEntityRenderState state, MatrixStack matrices, OrderedRenderCommandQueue queue, CameraRenderState cameraState) {
        queue.submitCustom(
                matrices,
                this.getLayer(),
    ...
    public void updateRenderState(
        ...
        for (Direction direction : Direction.values()) {
            try {
                if (!world.getBlockState(entity.getPos().offset(direction, 1)).getBlock().equals(ModBlock.NOBLUE_BLOCK)) {
